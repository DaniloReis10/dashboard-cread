  ...
  const fetchData = async (inicio, fim) => {
    ...
    try {
      ...
      const response = await fetch(
        `${process.env.REACT_APP_API_URL}/matriculas/total_yearly`,
        {
          method: "GET",
          headers: {
      ...
      // (Optional) We now filter by year here, in the browser, instead of in the API call.
      const filteredJson = json.filter(item => item.ano >= inicio && item.ano <= fim);

      console.log("Dados recebidos:", filteredJson);
